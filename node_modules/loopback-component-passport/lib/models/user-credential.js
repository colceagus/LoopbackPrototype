...
module.exports = function(UserCredential) {
  ...
  UserCredential.link = function(userId, provider, authScheme, profile,
    ...
    userCredentialModel.find({where: {
      provider: provider,
      ...
      if (extCredential && extCredential.length === 1) {
        extCredential = extCredential[0];
        if (extCredential.userId !== userId) {
          return cb(g.f('The account is already linked to another user.'));
        }
